...
data = {
    'Employer': ['My Realtor', 'Roshon', 'Amar', 'Lohith', 'Ashwini', 'Mohan', 'Pramod'],
    'Department': ['Deploymind', 'A', 'EC', 'FC', 'E', 'ES', 'The Guitar'],
    'Experience': [10, 15, 5, 14, 20, 14, 5],
    'Type': ['Regular', 'Adhoc', 'Regular', 'Contract', 'Regular', 'Contract', 'Adhoc'],
    'Salary': [50000, 20000, 15000, 30000, 40000, 15000, 10000]
}
...
plt.title('Total Salary by Type and Department')
plt.legend(title='Department')
plt.show()
pivot_salary_summary['mean'].plot(kind='bar', stacked=True, figsize=(10, 6))
plt.xlabel('Type')
plt.ylabel('Average Salary')
plt.title('Average Salary by Type and Department')
plt.legend(title='Department')
plt.show()